...
## 最新执行状态：软件 K=2/K=3/K=4 基线完成，K=2 加速完成
...
当前待做的是 K=3/K=4 加速硬件移植、RTL 官方 KAT 和 Vivado 实现。
...
CPU-only/CPU+PQC 对照显示当前端到端为 0.9930×，下一步重点转向接口和批量搬运优化。
...
路径已验证，快速 CPU 的 145 条官方 KAT 已完成；当前证据表明必须先降低搬运和轮询开销，
再扩展多 PE 或更大参数集。
...
RV32IM-fast 的官方 KAT 对照已完成；当前结果用于定位搬运/轮询瓶颈，实板仍放到最后。

| 工作范围 | ML-KEM-512 | ML-KEM-768 / 1024 |
|---|---|---|
| NIST 安全类别 | 1，主目标应用的安全等级 | 3 / 5，扩展参数集 |
| 官方公开向量正确性回归 | 三种 CPU 各 145 条已完成 | 主机已通过；768 fast CPU 145/145，1024 fast CPU 145/145，其他CPU配置待测 |
...
| 标准软件的完整 CPU baseline | 512 API/阶段分析和768/1024 RV32IM-fast CPU全集完成；其他CPU消融和K3/K4硬件待做 | [M2 路线](COMPETITION_ROADMAP.md#m2picorv32-完整软件-baseline) |
...
本次另新增768 fast CPU的145项。768/1024均为RV32IM-fast CPU-only覆盖；不能据此称K3/K4加速硬件已通过。
...
| 4a | 独立 HLS＋BRAM＋MMIO 验证 | RTL、BIST、布局布线与匹配 XPR/bitstream | 已完成；CPU 接入后仍保留独立证据 |
| 5 | 快速 CPU＋多项式加速器主线接通 | 完整 512 官方回归及未插桩 API 软件/硬件对照，计入搬运/等待 | 已完成；145/145，端到端 0.9930× |
...
| 9 | 其他 CPU 消融及 768/1024 扩展 | 补齐官方回归、基本周期/内存、最终架构验证及适用实现结果 | 768/1024 RV32IM-fast CPU完成；RV32I/迭代CPU消融及K3/K4硬件待测 |
...
> MMIO/BRAM/BIST、Vivado 2024.2 实现及 CPU+PQC 全流程已完成。K=3/K=4 加速硬件
> 仍待移植、RTL 官方 KAT 和 Vivado 实现验证。
