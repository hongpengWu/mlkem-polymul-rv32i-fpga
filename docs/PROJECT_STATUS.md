...
运算域、缩放和缓存向量乘加接口已经审计，并已按标准接口新建独立的 K=2 BaseMul
HLS 原型，下一步再接通完整 KEM。
...
> 两种 Decaps 形式及密钥检查。快速乘法 CPU 的 512 全量阶段分析也已完成；
> 768/1024 CPU 回归、64 KiB 实现及标准软件的硬件加速对照仍待完成。
