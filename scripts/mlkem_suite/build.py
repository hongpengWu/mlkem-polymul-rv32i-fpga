...
def main():
    ...
    require(ram_bytes == 131072 and stack_bytes == 32768,
            'The generic RTL runner requires 128 KiB RAM and 32 KiB stack')
    ...
    source_files = [Path(__file__), ROOT / 'scripts/kat/package_mlkem.py',
                    *src.glob('*'), *fixtures.glob('*'),
                    *[common / n for n in ('startup.S', 'runtime.c', 'kat_protocol.h')],
                    vendor / 'SOURCE_MANIFEST.json', *[vendor / e['path'] for e in manifest['files']]]
    ...
    for isa in ('rv32i', 'rv32im'):
        ...
        for api in ('keypair_derand', 'enc_derand', 'dec', 'check_pk', 'check_sk'):
            require(f'pico_mlkem{level}_{api}' in symbols, f'Missing API: {api}')
        ...
        record = dict(parameter_set=level, isa=isa, compiler=version, ram_bytes=ram_bytes, stack_reserved=stack_bytes,
                      ...
                      m_instructions_by_function=by_function, flags=flags, commands=commands,
                      source_sha256=source_hashes, mlkem_native_commit=manifest['source_commit'],
                      linker_text=linker.read_text(), linker_sha256=digest(linker),
                      compiler_sha256=digest(tool('gcc')), libgcc_path=str(libgcc), libgcc_sha256=digest(libgcc))
    ...
    for name, wanted in source_hashes.items():
        require(digest(ROOT / name) == wanted, f'Build input changed: {name}')
