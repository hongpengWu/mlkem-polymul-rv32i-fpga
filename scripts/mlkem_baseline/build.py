"""Build one official ML-KEM-512 KeyGen case for RV32I and RV32IM."""
...
def main():
    ...
    src = ROOT / 'firmware/mlkem_baseline'
    fixtures = ROOT / 'tb/software/mlkem_keygen'
    build = ROOT / 'build/mlkem_baseline'
    images = ROOT / 'firmware/images/mlkem_keygen'
    ...
    print(run([sys.executable, ROOT / 'scripts/kat/package_keygen.py',
               '--parameter-set', '512', '--tc-id', '1', '--out-dir', fixtures]).strip())
    ...
    sources = [src / 'startup.S', src / 'kat_keygen.c', src / 'runtime.c']
    ...
    for isa in ['rv32i', 'rv32im']:
        ...
        flags = [f'-march={isa}', '-mabi=ilp32', '-O3', '-std=c11', '-ffreestanding',
                 ...
                 '-Wall', '-Wextra', '-Werror', '-I', str(src), '-I', str(fixtures),
        ...
        for path in sources:
            ...
            objects.append(obj)
        elf, binary = out / 'mlkem_keygen.elf', out / 'mlkem_keygen.bin'
        command = [tool('gcc'), *flags, '-nostdlib', '-nostartfiles',
                   '-Wl,--build-id=none', '-Wl,--gc-sections',
                   f'-Wl,-Map,{out / "mlkem_keygen.map"}', '-T', src / 'link.ld',
                   *objects, '-lgcc', '-o', elf]
        logs.append(run(command))
        commands.append([str(x) for x in command])
        ...
        require('pico_mlkem512_keypair_derand' in symbols, 'Missing real KeyGen implementation')
