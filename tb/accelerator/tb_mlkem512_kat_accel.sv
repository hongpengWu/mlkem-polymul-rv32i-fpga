...
// remain in this testbench; only K=2 BaseMul is executed through hardware.
module tb_mlkem512_kat_accel #(
  ...
  parameter FIRMWARE_INIT_FILE="kat.mem",
  parameter integer EXPECT_ACCEL=1
);
  ...
  always #5 clk=~clk;
  integer hw_starts=0, hw_done=0, hw_loads=0, hw_reads=0;
  longint hw_core=0, hw_load_cycles=0, hw_read_cycles=0;
  longint load_begin=0, read_begin=0, core_begin=0;
  mlkem512_accel_system #(
    .FIRMWARE_INIT_FILE(FIRMWARE_INIT_FILE), .RAM_ADDR_BITS(14),
  ...
  function automatic valid_address(input [31:0] address);
    valid_address=address<RAM_LIMIT || address[31:6]==DEBUG_BASE[31:6] || (address>=32'h50001000 && address<32'h50003000);
  endfunction
  ...
  always @(posedge clk) begin
    if (resetn) begin
      ...
      if(dut.adapter_i.error) $fatal(1,"MLKEM512_FAIL accelerator error");
      if(dut.adapter_i.fire) begin
        if(!algorithm_active) $fatal(1,"MLKEM512_FAIL accelerator access outside API");
        if(dut.adapter_i.write_input && |dut.adapter_i.bus_wstrb) begin
          if(hw_loads%640==0) load_begin=cycle_count;
          hw_loads=hw_loads+1;
          if(hw_loads%640==0) hw_load_cycles=hw_load_cycles+cycle_count-load_begin+1;
        end
        if(dut.adapter_i.bus_write && dut.adapter_i.offset=='h1000 && dut.adapter_i.bus_wdata[0]) begin
          hw_starts=hw_starts+1; core_begin=cycle_count;
        end
        if(!dut.adapter_i.bus_write && dut.adapter_i.result_window) begin
          if(hw_reads%128==0) read_begin=cycle_count;
          hw_reads=hw_reads+1;
          if(hw_reads%128==0) hw_read_cycles=hw_read_cycles+cycle_count-read_begin+1;
        end
      end
      if(dut.adapter_i.busy && dut.adapter_i.hls_done) begin
        hw_done=hw_done+1; hw_core=hw_core+cycle_count-core_begin;
      end
      ...
      if (dut.local_write_commit && dut.commit_addr[31:6]==DEBUG_BASE[31:6]) begin
        ...
        if (algorithm_active) $fatal(1,"MLKEM512_FAIL debug write inside timed algorithm");
        if (dut.commit_addr==DEBUG_BASE+60) begin
          case (dut.commit_data)
            ...
            32'h100: begin
              ...
              hw_starts=0; hw_done=0; hw_loads=0; hw_reads=0;
              hw_core=0; hw_load_cycles=0; hw_read_cycles=0;
            ...
            32'h105: begin
              ...
              if(hw_done!=hw_starts || hw_loads!=640*hw_starts || hw_reads!=128*hw_starts || hw_core!=136*hw_starts)
                $fatal(1,"MLKEM512_FAIL incomplete hardware calls");
              if(EXPECT_ACCEL && ((input_fixture[current_input_offset+5]<=4 && hw_starts==0) || (input_fixture[current_input_offset+5]>4 && hw_starts!=0)))
                $fatal(1,"MLKEM512_FAIL hardware not used as expected");
              if(!EXPECT_ACCEL && hw_starts!=0) $fatal(1,"MLKEM512_FAIL software control used hardware");
              $display("ACCEL_METRICS index=%0d starts=%0d done=%0d loads=%0d reads=%0d core_cycles=%0d load_cycles=%0d read_cycles=%0d",current_case,hw_starts,hw_done,hw_loads,hw_reads,hw_core,hw_load_cycles,hw_read_cycles);
