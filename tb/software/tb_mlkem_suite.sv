...
module tb_mlkem_suite;
  ...
  parameter integer PARAMETER_SET=512;
  parameter integer RAM_ADDR_BITS=14;
  parameter integer STACK_BYTES=16384;
  parameter FIRMWARE_INIT_FILE="mlkem512.mem";
  parameter INPUT_FIXTURE_FILE="mlkem512_input.mem";
  parameter EXPECTED_FIXTURE_FILE="mlkem512_expected.mem";

  localparam integer MAX_CASES=145, MAX_WORDS=131072;
  localparam integer PK_BYTES=(PARAMETER_SET==512) ? 800 : ((PARAMETER_SET==768) ? 1184 : 1568);
  localparam integer SK_BYTES=(PARAMETER_SET==512) ? 1632 : ((PARAMETER_SET==768) ? 2400 : 3168);
  localparam integer CT_BYTES=(PARAMETER_SET==512) ? 768 : ((PARAMETER_SET==768) ? 1088 : 1568);
  localparam [31:0] RAM_LIMIT=(32'h1 << (RAM_ADDR_BITS+2));
  localparam [31:0] STACK_TOP=RAM_LIMIT-32'd16, STACK_BOTTOM=STACK_TOP-STACK_BYTES;
  ...
  cpu_benchmark_system #(
    .FIRMWARE_INIT_FILE(FIRMWARE_INIT_FILE), .RAM_ADDR_BITS(RAM_ADDR_BITS),
  ...
  task automatic require_state(input integer wanted, input integer event_code);
    begin
      if (state!=wanted)
        $fatal(1,"MLKEM_FAIL event=%h state=%0d expected=%0d case=%0d cycle=%0d",event_code,state,wanted,current_case,cycle_count);
      if (event_code!=32'h1ff && status!==STATUS_RUN)
        $fatal(1,"MLKEM_FAIL event without RUN status=%h",status);
      if (dut.debug_regs[1]!==0)
        $fatal(1,"MLKEM_FAIL firmware error=%h",dut.debug_regs[1]);
      if (event_code!=32'h100 && event_code!=32'h200 && event_code!=32'h1ff) begin
        if (dut.debug_regs[2]!==current_case ||
          ...
          $fatal(1,"MLKEM_FAIL metadata changed during case=%0d",current_case);
      end
    end
  endtask
  ...
  task automatic check_stream(input integer is_output);
    ...
    begin
      ...
      if (stream_words>=byte_count/4 || dut.debug_regs[12]!==stream_words || dut.debug_regs[14]!==4)
        $fatal(1,"MLKEM_FAIL stream shape case=%0d output=%0d word=%0d index=%h valid=%h",current_case,is_output,stream_words,dut.debug_regs[12],dut.debug_regs[14]);
      ...
      for (b=0;b<4;b=b+1) begin
        if (payload[8*b+:8]!==wanted[8*b+:8])
          $fatal(1,"MLKEM_FAIL byte mismatch case=%0d output=%0d byte=%0d got=%h expected=%h",current_case,is_output,4*stream_words+b,payload[8*b+:8],wanted[8*b+:8]);
  ...
  task automatic check_metrics;
    begin
      if (rdcycle_samples!=2 || algorithm_active || algorithm_cycles<=empty_cycles ||
        ...
        $fatal(1,"MLKEM_FAIL cycle boundary case=%0d samples=%0d firmware=%h observed=%0d empty=%0d",current_case,rdcycle_samples,dut.debug_regs[3],algorithm_cycles,empty_cycles);
      if (dut.debug_regs[9]!==expected_fixture[current_expected_offset+11])
        $fatal(1,"MLKEM_FAIL API return case=%0d got=%h expected=%h",current_case,dut.debug_regs[9],expected_fixture[current_expected_offset+11]);
      ...
      if ((EXPECT_M==0 && m_total!=0) ||
        ...
        $fatal(1,"MLKEM_FAIL unexpected M execution case=%0d expected=%0d total=%0d",current_case,EXPECT_M,m_total);
    end
  endtask
  ...
  initial begin
    if ((EXPECT_M!=0 && EXPECT_M!=1) ||
      ...
      $fatal(1,"MLKEM_FAIL invalid CPU/firmware configuration");
    for (j=0;j<8;j=j+1) m_count[j]=0;
    vector_file=$fopen(INPUT_FIXTURE_FILE,"r");
    if (!vector_file) $fatal(1,"MLKEM_FAIL missing input fixture");
    while (!$feof(vector_file)) begin
      scan_result=$fscanf(vector_file,"%h",file_word);
      if (scan_result==1) begin
        if (input_words_read>=MAX_WORDS || (^file_word)===1'bx)
          $fatal(1,"MLKEM_FAIL oversized/unknown input fixture");
        input_fixture[input_words_read]=file_word; input_words_read=input_words_read+1;
      end else if (!$feof(vector_file)) $fatal(1,"MLKEM_FAIL malformed input fixture");
    end
    $fclose(vector_file);
    vector_file=$fopen(EXPECTED_FIXTURE_FILE,"r");
    if (!vector_file) $fatal(1,"MLKEM_FAIL missing expected fixture");
    while (!$feof(vector_file)) begin
      scan_result=$fscanf(vector_file,"%h",file_word);
      if (scan_result==1) begin
        if (expected_words_read>=MAX_WORDS || (^file_word)===1'bx)
          $fatal(1,"MLKEM_FAIL oversized/unknown expected fixture");
        expected_fixture[expected_words_read]=file_word; expected_words_read=expected_words_read+1;
      end else if (!$feof(vector_file)) $fatal(1,"MLKEM_FAIL malformed expected fixture");
    end
    $fclose(vector_file);
    if (input_words_read<8 || expected_words_read<8) $fatal(1,"MLKEM_FAIL truncated header");
    ...
    if (EXPECTED_CASES<1 || EXPECTED_CASES>MAX_CASES || fixture_cases!=EXPECTED_CASES || input_fixture[4]!==input_words_read || expected_fixture[4]!==expected_words_read)
      $fatal(1,"MLKEM_FAIL fixture size/case count mismatch");
    for (j=0;j<8;j=j+1)
      if (j!=4 && input_fixture[j]!==expected_fixture[j]) $fatal(1,"MLKEM_FAIL header mismatch word=%0d",j);
    if (input_fixture[0]!==32'h3254414b || input_fixture[1]!==1 || input_fixture[2]!==PARAMETER_SET ||
        input_fixture[5]!==0 || input_fixture[6]!==0 || input_fixture[7]!==0)
      $fatal(1,"MLKEM_FAIL invalid header");
    pos_in=8; pos_out=8;
    for (j=0;j<fixture_cases;j=j+1) begin
      if (pos_in+12>input_words_read || pos_out+12>expected_words_read)
        $fatal(1,"MLKEM_FAIL truncated metadata case=%0d",j);
      input_offsets[j]=pos_in; expected_offsets[j]=pos_out;
      for (k=0;k<11;k=k+1)
        if (input_fixture[pos_in+k]!==expected_fixture[pos_out+k])
          $fatal(1,"MLKEM_FAIL metadata mismatch case=%0d word=%0d",j,k);
      if (input_fixture[pos_in]!==j || input_fixture[pos_in+11]!==0 ||
          input_fixture[pos_in+5]<1 || input_fixture[pos_in+5]>6)
        $fatal(1,"MLKEM_FAIL invalid case identity/operation case=%0d",j);
      for (k=6;k<11;k=k+1)
        if (input_fixture[pos_in+k]>4096 || input_fixture[pos_in+k]%4!=0)
          $fatal(1,"MLKEM_FAIL invalid payload size case=%0d word=%0d",j,k);
      case (input_fixture[pos_in+5])
        1: if (input_fixture[pos_in+6]!==32 || input_fixture[pos_in+7]!==32 || input_fixture[pos_in+8]!==0 || input_fixture[pos_in+9]!==PK_BYTES || input_fixture[pos_in+10]!==SK_BYTES) $fatal(1,"MLKEM_FAIL KeyGen shape");
        2: if (input_fixture[pos_in+6]!==PK_BYTES || input_fixture[pos_in+7]!==32 || input_fixture[pos_in+8]!==0 || input_fixture[pos_in+9]!==CT_BYTES || input_fixture[pos_in+10]!==32) $fatal(1,"MLKEM_FAIL Encaps shape");
        3: if (input_fixture[pos_in+6]!==SK_BYTES || input_fixture[pos_in+7]!==CT_BYTES || input_fixture[pos_in+8]!==0 || input_fixture[pos_in+9]!==32 || input_fixture[pos_in+10]!==0) $fatal(1,"MLKEM_FAIL Decaps shape");
        4: if (input_fixture[pos_in+6]!==32 || input_fixture[pos_in+7]!==32 || input_fixture[pos_in+8]!==CT_BYTES || input_fixture[pos_in+9]!==32 || input_fixture[pos_in+10]!==0) $fatal(1,"MLKEM_FAIL seeded Decaps shape");
        5: if (input_fixture[pos_in+6]!==PK_BYTES || input_fixture[pos_in+7]!==0 || input_fixture[pos_in+8]!==0 || input_fixture[pos_in+9]!==0 || input_fixture[pos_in+10]!==0) $fatal(1,"MLKEM_FAIL public-key check shape");
        6: if (input_fixture[pos_in+6]!==SK_BYTES || input_fixture[pos_in+7]!==0 || input_fixture[pos_in+8]!==0 || input_fixture[pos_in+9]!==0 || input_fixture[pos_in+10]!==0) $fatal(1,"MLKEM_FAIL secret-key check shape");
      endcase
      if (expected_fixture[pos_out+11]!==0 && expected_fixture[pos_out+11]!==32'hfffffffc && expected_fixture[pos_out+11]!==32'hfffffffb)
        $fatal(1,"MLKEM_FAIL unsupported expected API return case=%0d",j);
      ...
      if (pos_in>input_words_read || pos_out>expected_words_read) $fatal(1,"MLKEM_FAIL truncated payload case=%0d",j);
    end
    if (pos_in!=input_words_read || pos_out!=expected_words_read) $fatal(1,"MLKEM_FAIL trailing fixture data");
    $display("MLKEM_HEADER,index,dataset,vsId,tgId,tcId,op,return_code,raw_cycles,empty_cycles,input_bytes,output_bytes,m_total,mul,mulh,mulhsu,mulhu,div,divu,rem,remu,min_sp");
    $display("MLKEM_START cases=%0d ram_bytes=%0d stack_bytes=%0d clock_mhz=100 mul=%0d fast_mul=%0d div=%0d expect_m=%0d",fixture_cases,RAM_LIMIT,STACK_BYTES,CPU_ENABLE_MUL,CPU_ENABLE_FAST_MUL,CPU_ENABLE_DIV,EXPECT_M);
  ...
  always @(posedge clk) begin
    if (resetn) begin
      ...
      if (cycle_count-case_start_cycle>=TIMEOUT_CYCLES)
        $fatal(1,"MLKEM_FAIL timeout cycle=%0d case=%0d state=%0d pc=%h sp=%h",cycle_count,current_case,state,dut.cpu.picorv32_core.reg_pc,observed_sp);
      if (trap!==1'b0) $fatal(1,"MLKEM_FAIL CPU trap=%b pc=%h",trap,dut.cpu.picorv32_core.reg_pc);
      if (status===STATUS_FAIL) $fatal(1,"MLKEM_FAIL firmware error=%h",dut.debug_regs[1]);
      if (dut.c_arvalid===1'b1 && valid_address(dut.c_araddr)!==1'b1) $fatal(1,"MLKEM_FAIL out-of-map read %h",dut.c_araddr);
      if (dut.c_awvalid===1'b1 && valid_address(dut.c_awaddr)!==1'b1) $fatal(1,"MLKEM_FAIL out-of-map write %h",dut.c_awaddr);
      ...
      if (stack_monitor_active) begin
        if ((^observed_sp)===1'bx || observed_sp<STACK_BOTTOM || observed_sp>STACK_TOP || observed_sp[3:0]!=0)
          $fatal(1,"MLKEM_FAIL invalid stack pointer %h case=%0d",observed_sp,current_case);
      ...
      if (rdcycle_executing) begin
        if (state==MEASURE) begin
          if (rdcycle_samples==0) begin
            ...
            algorithm_active=0;
          end else $fatal(1,"MLKEM_FAIL extra rdcycle in algorithm");
        ...
        end else $fatal(1,"MLKEM_FAIL rdcycle outside measurement window");
      end
      if (dut.cpu.picorv32_core.pcpi_valid && dut.cpu.picorv32_core.pcpi_int_ready) begin
        if (observed_insn[31:25]!==7'b0000001 || observed_insn[6:0]!==7'b0110011 || (^observed_insn[14:12])===1'bx)
          $fatal(1,"MLKEM_FAIL unexpected PCPI instruction %h",observed_insn);
        operation=observed_insn[14:12]; all_m_total=all_m_total+1;
        if (EXPECT_M==0) $fatal(1,"MLKEM_FAIL RV32I executed M instruction");
      ...
      if (dut.local_write_commit && dut.commit_addr[31:6]==DEBUG_BASE[31:6]) begin
        if (dut.commit_strb!==4'b1111 || dut.commit_addr[1:0]!==0) $fatal(1,"MLKEM_FAIL partial/unaligned debug write");
        if (dut.commit_addr==DEBUG_BASE+40 || dut.commit_addr==DEBUG_BASE+44) $fatal(1,"MLKEM_FAIL CPU wrote host mailbox");
        if (algorithm_active) $fatal(1,"MLKEM_FAIL debug write inside timed algorithm");
          ...
          case (dut.commit_data)
            32'h200: begin
              ...
              if (pending_request!=-1 || requested_words>=input_words_read || completed_cases>=fixture_cases ||
                ...
                $fatal(1,"MLKEM_FAIL invalid mailbox request requested=%0d index=%h case=%0d",requested_words,dut.debug_regs[12],completed_cases);
            ...
            32'h100: begin
              ...
              if (completed_cases>=fixture_cases || pending_request!=-1 || !stack_monitor_active || calibration_samples!=2)
                $fatal(1,"MLKEM_FAIL premature case begin");
              ...
              if (requested_words!=current_input_offset+12+input_sizes[current_case]/4 ||
                ...
                $fatal(1,"MLKEM_FAIL invalid case metadata/calibration case=%0d",current_case);
            ...
            32'h108: begin
              ...
              if (case_input_checks!=input_sizes[current_case] || rdcycle_samples!=0 || algorithm_active)
                $fatal(1,"MLKEM_FAIL premature/nested measurement case=%0d",current_case);
              state=MEASURE;
            end
            32'h109: begin
              require_state(MEASURE,32'h109);
              if (rdcycle_samples!=2 || algorithm_active || algorithm_cycles<=empty_cycles)
                $fatal(1,"MLKEM_FAIL missing/invalid rdcycle pair case=%0d",current_case);
            ...
            32'h105: begin
              ...
              if (case_input_checks!=input_sizes[current_case] || case_output_checks!=output_sizes[current_case])
                $fatal(1,"MLKEM_FAIL incomplete case bytes case=%0d",current_case);
              $display("MLKEM_ROW,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,0x%08h",
            ...
            32'h1ff: begin
              ...
              if (completed_cases!=fixture_cases || input_checks!=expected_input_bytes || output_checks!=expected_output_bytes ||
                ...
                $fatal(1,"MLKEM_FAIL premature completion cases=%0d inputs=%0d outputs=%0d requests=%0d status=%h",completed_cases,input_checks,output_checks,requested_words,status);
              $display("MLKEM_PASS cases=%0d input_bytes=%0d output_bytes=%0d cpu_mul=%0d cpu_fast_mul=%0d cpu_div=%0d expect_m=%0d cycles=%0d all_m=%0d min_sp=0x%08h stack_used=%0d",
            ...
            default: $fatal(1,"MLKEM_FAIL unknown event=%h",dut.commit_data);
          endcase
        end
      end
    end
