/* Generic deterministic ML-KEM KAT driver for one compile-time level. */
#include <stdint.h>
...
void kat_main(void)
{
    ...
    if (header[0] != 0x3254414b || header[1] != 1 || header[2] != MLK_CONFIG_PARAMETER_SET ||
        header[3] == 0 || header[3] > 145 || header[4] < 8 || header[5] || header[6] || header[7]) fail(1);
    ...
    for (uint32_t c = 0; c < header[3]; ++c) {
        ...
        stop = kat_cycle(); KAT_DEBUG_EVENT = KAT_EVENT_MEASURE_END; KAT_DEBUG_CYCLES = stop - start; KAT_DEBUG_AUX5 = (uint32_t)result;
